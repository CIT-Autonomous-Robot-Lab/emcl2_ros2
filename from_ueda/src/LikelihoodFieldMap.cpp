...
LikelihoodFieldMap::LikelihoodFieldMap(const nav_msgs::OccupancyGrid & map, double likelihood_range)
{
  ...
  for (int x = 0; x < width_; x++) {
    ...
    for (int y = 0; y < height_; y++) likelihoods_[x][y] = 0.0;
  }
...
{
  for (auto & e : likelihoods_) delete[] e;
}
...
double LikelihoodFieldMap::likelihood(double x, double y)
{
  ...
  if (ix < 0 or iy < 0 or ix >= width_ or iy >= height_) return 0.0;
...
void LikelihoodFieldMap::setLikelihood(int x, int y, double range)
{
  ...
  for (int i = 0; i <= cell_num; i++) weights.push_back(1.0 - (double)i / cell_num);
    ...
    for (int j = -cell_num; j <= cell_num; j++)
      if (i + x >= 0 and j + y >= 0 and i + x < width_ and j + y < height_)
        likelihoods_[i + x][j + y] =
          std::max(likelihoods_[i + x][j + y], std::min(weights[abs(i)], weights[abs(j)]));
}
...
void LikelihoodFieldMap::normalize(void)
{
  double maximum = 0.0;
  for (int x = 0; x < width_; x++)
    for (int y = 0; y < height_; y++) maximum = std::max(likelihoods_[x][y], maximum);

  for (int x = 0; x < width_; x++)
    for (int y = 0; y < height_; y++) likelihoods_[x][y] /= maximum;
}
